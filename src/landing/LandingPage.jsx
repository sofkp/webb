...
const LandingPage = () => {
  ...
  useEffect(() => {
    const fetchTenants = async () => {
      try {
        const response = await fetch('https://3pmkubnr13.execute-api.us-east-1.amazonaws.com/prod/customization/list-logos');
  ...
  const fetchInventory = async (tenantId) => {
    try {
      const response = await fetch(
        `https://lzd8yyhab1.execute-api.us-east-1.amazonaws.com/prod/inventory/names?tenant_id=${tenantId}`
      );
      const data = await response.json();
      if (response.ok) {
